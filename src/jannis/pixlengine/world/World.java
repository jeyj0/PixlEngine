...
public class World {
	 ...
	 * @param x Amount of horizontal chunks
	 * @param y Amount of vertical chunks
	 */
	 ...
	 * @param x The chunk's horizontal chunk coordinate
	 * @param y The chunk's vertical chunk coordinate
	 * @return The chunk at the given chunk coordinates or null on invalid argument
	 */
	...
	public Field getField(int x, int y) {
		...
		int chunkX = (x - x % PixlEngine.getChunkSize()) / PixlEngine.getChunkSize();
		if (chunkX >= chunks.length)
			return null;

		// get the chunk y-coordinate
		int chunkY = (y - y % PixlEngine.getChunkSize()) / PixlEngine.getChunkSize();
		if (chunkY >= chunks[chunkX].length)
			return null;
		
		return chunks[chunkX][chunkY].getField(x % PixlEngine.getChunkSize(), y % PixlEngine.getChunkSize());
	}
